...
# start_time = time.perf_counter()
# new_html_list = asyncio.run(fast_spider(active_header, "https://www.forexfactory.com/thread/958677-crude-oil")) # running the crawler
# stop_time = time.perf_counter()
...
async def get_url(thread_url, header):
    ...
    if res.status_code == 200:
        ...
        if page_num_tag:
            ...
            return url_list.append(thread_url)
...
# run the get_url
start_time = time.perf_counter()
url_list = asyncio.run(get_url("https://www.forexfactory.com/thread/145348-need-help-and-explanation-on-interpretation-of-news", active_header)) # running the crawler
stop_time = time.perf_counter()
